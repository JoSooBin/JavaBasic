...
public class SungJukV10DAO {
    ...
    public static String insertSungJuk(SungJukVO sj) {
        ...
        try {
            ...
            System.out.println("insertSQL에서 오류 발생");
        }
        SungJukJDBC.destoryConn(conn,pstmt);
        return result;
    }

    //번호,이름,국어,영어,수학,등록일 조회하고 그 결과들을 ArrayList에 담아서 넘김
    ...
    //성적번호로 성적데이터를 조회하고 그 결과를 SungJukVO에 담아서 넘김
    ...
    //수정할 성적번호를 매개변수로 넘겨주면 성적 테이블에서 해당 데이터를 수정함
    ...
    //삭제할 성적번호를 매개변수로 넘겨주면 sungjuk테이블에서 해당 데이터를 삭제함
    public static String deleteSungJuk(int sjno) {
        ...
        conn = SungJukJDBC.makeConn();
        try {
            pstmt = conn.prepareStatement(SungJukJDBC.delectSungJuk);
            pstmt.setInt(1,sjno);
