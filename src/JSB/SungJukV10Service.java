...
import static JSB.SungJukJDBC.selectSungJuk;
...
public class SungJukV10Service {
    ...
    public void newSungJuk() {
        String name;
        int sjno = 0,kor, eng, mat,regdate = 0;
       ...
       SungJukVO sj = new SungJukVO(sjno,name, kor, eng, mat,0,0,' ',"");

        //총점, 평균, 학점을 계산
        computeSungJuk(sj);

        //최종적으로 처리한 성적데이터를 sungjuk테이블에 저장하기 위해
        //SungJukV10DAO의 insertSungJuk메서드를 호출하고 매개변수로 성적데이터를 넘김
    ...
    public void readOneSungJuk() {
        String fmt = "번호:%d, 이름: %s, 국어: %d, 수학: %d, 영어: %d\n " +
        ...
        String result = String.format(fmt,sj.getSjno(), sj.getName(),sj.getKor(),sj.getEng(),
                sj.getMat(),sj.getSum(),sj.getMean(),sj.getGrd(), sj.getRegdate());
        System.out.println(result);
    }
